...
By computing the above lieklihoods of each tuple moving onto the following week, we can then run the Opportunty Cost model on the set of picks, taking the pick with the largest difference between the best and second best pick across all weeks we are considering. 
...
As above, we can compare the performance of the OC model with the Greedy Model in the Multi-Entry case. The below plot is in terms of the likelihood of moving onto the following week, and so a higher is better. As with teh Single-Entry case, we can see the OC model trading off some early likelihood for greater future lieklihoods of having at least one team move onto the next week. 
